...
export default function Header() {
  ...
  const navDropdown = (to, text) =>  <NavLink className="nav-link dropdown-item text-dark" to={to}><i class="fas fa-sign-in-alt me-3 icnav"></i>{text}</NavLink>
  const cartNull = () => <div className ={`dropdown-menu submenu bg-white text-center mt-4 ${openCart}`}> <h5>Giỏ hàng đang rỗng</h5></div>
      ...
      <div className={`dropdown-menu submenu ticket p-0 mt-4 bg-light ${openCart}`} style={{border:'none'}} >
        ...
        <div className="bg-white p-2 pb-3 pt-3 text-center">
          <div className="d-flex justify-content-between font-weight-bold mb-3">
            ...
            <h5>{danhSachGheDangDat.reduce((sum, seatActive) => sum += seatActive.giaVe, 0).toLocaleString() + " đ"}</h5>
          </div>
        ...
        <div className="container d-flex">
          ...
          <div class="offcanvas offcanvas-end col-xl-7 " tabindex="-1" id="offcanvasNavbar" aria-labelledby="offcanvasNavbarLabel">
            ...
            <ul className="navbar-nav mx-auto border-bottom text-center">
              ...
              <li className="nav-item px-3 dropdown" onClick={()=>{setOpenCart('')}}><a className="nav-link text-white" href="#" id="a"  data-bs-toggle="dropdown" role="button" aria-expanded="false"><i className='bx bx-id-card icnav'></i>{account !== '' ? account.taiKhoan : 'PR0FILE'}<i className='bx bxs-down-arrow ps-3 pt-1'></i></a>
                <ul className="dropdown-menu bg-white submenu mt-4" aria-labelledby="a">
                  <li className="nav-item px-3">{account === '' ? navDropdown('/login','SIGN IN') : navDropdown('/profile','PR0FILE')}</li>
                  ...
                  {account.maLoaiNguoiDung === 'QuanTri' ? <li className="nav-item px-3">{navDropdown('/admin','ADMIN')}</li> : null}
                  {account !== '' ? <li className="nav-item px-3"><a className="nav-link dropdown-item text-dark" onClick ={()=>{dispatch({type:'LOG_OUT'})}} href="#"><i class="fas fa-sign-out-alt me-3 icnav"></i>LOG OUT</a></li> : null}
                </ul>
